...
public class UIPickableFormRenderer extends UIFormRenderer
{
    ...
    private void renderAxes(UIContext context)
    {
        ...
        if (UIBaseMenu.renderAxes)
        {
            ...
            // 3 colored rotation rings (solid)
            float ringRadius = baseRadius;
            float band = 0.03F; // thinner

            // XY plane (Z rotation) - blue
            Draw.renderRing(stack, ringRadius, band, 96, 0F, 0F, 1F, 0.95F);
            // XZ plane (Y rotation) - green
            stack.push();
            stack.multiply(RotationAxis.POSITIVE_X.rotationDegrees(90F));
            Draw.renderRing(stack, ringRadius, band, 96, 0F, 1F, 0F, 0.95F);
            stack.pop();
            // YZ plane (X rotation) - red
            stack.push();
            stack.multiply(RotationAxis.POSITIVE_Y.rotationDegrees(90F));
            Draw.renderRing(stack, ringRadius, band, 96, 1F, 0F, 0F, 0.95F);
            stack.pop();
